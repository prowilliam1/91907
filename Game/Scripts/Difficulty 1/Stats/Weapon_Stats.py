...
# Define weapons and their effects on player stats
weapons = {
    "Sword": {"attack": 1, "special_attack": 50, "special_equation": lambda x, y: x + y},
...
# Define special questions for each weapon
special_questions = {
    "Sword": [
        {
            "question": lambda x, y, v, w, z: f"{x}x + {y} + {w}^{v} = {z}",
            "answer": lambda x, y, v, w, z: (z - y - w**v) // x,
            "adjust_z": lambda x, y, v, w: x * random.randint(1, 10) + y + w**v,  # Ensure z is divisible by x
        },
        {
            "question": lambda x, y, v, w, z: f"{x}x + {y} + {w}^{v} = {z}",
            "answer": lambda x, y, v, w, z: (z - y - w**v) // x,
